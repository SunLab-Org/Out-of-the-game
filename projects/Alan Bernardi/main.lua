...
function love.load()
    larghezza = love.graphics.getWidth()
    altezza = love.graphics.getHeight()
...
function love.update(dt)
    ...
    if carro.vx > 0 and carro.x <= love.graphics.getWidth() - carro.w then 
        carro.x = carro.x + carro.vx * dt
    end
    ...
    if carro2.vx > 0 and carro2.x <= love.graphics.getWidth() - carro2.w then 
        carro2.x = carro2.x + carro2.vx * dt
    end
    ...
    for index, c in ipairs(colpi) do
        ...
        if c.x <= 0 or c.x >= love.graphics.getWidth() then
            table.remove(colpi, index)
        end
...
function love.draw()
    ...
    love.graphics.print("Vite: "..carro2.hp, love.graphics.getWidth() - 100, 0)

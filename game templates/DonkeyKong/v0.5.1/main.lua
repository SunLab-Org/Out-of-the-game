...
--[[
    Obiettivi:
    √ implementare il sistema di salto
    - implementare le scale
    √ creare i "nemici"
    - implementare il sistema di sconfitta
]]
 
local playerGenerator = require "player"
local platformGenerat = require "platform"
local barrelGenerator = require "barrel"
...
function love.load()
    ...
    table.insert(Game.entities, platformGenerat.new(700, 600, 1000, 20))
    table.insert(Game.entities, platformGenerat.new(1000, 600, 20, 200, "stair"))
    table.insert(Game.entities, barrelGenerator.new(300, 300, 20, 20))
...
function Game.checkCollision(a, b)
    ...
    if collides == true then
        ...
        -- aggiungiamo questo parametro per dire all'oggetto che controlla la collisione
        -- con cosa e' entrato in contatto
        coll.type = b.type
        coll.arg = {}

        -- in caso di scale ritorniamo le coordinate necessarie
        if coll.type == "stair" then
            coll.arg.staircoord = {x = b.x, y = b.y, height = b.height}
            return coll
        end

        -- implementiamo un sistema di maschere, in cui inseriremo le collisioni che non
        -- vogliamo calcolare
        if a.mask then
            for _, m in pairs(a.mask) do
                if m == b.type then return nil end
            end
        end
